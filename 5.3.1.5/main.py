...
print
print(
    "Distance of acceptable Ping speed from average: {:.2f} standard deviations".format(
        (quote_ping[0] - acceptable_ping) / quote_ping[1]
    )
)
# print('Distance of acceptable Download speed from average
print(
    "Distance of acceptable Download speed from average: {:.2f} standard deviations".format(
        (quote_download[0] - acceptable_download) / quote_download[1]
    )
)
# print('Distance of acceptable Upload speed from average
print(
    "Distance of acceptable Upload speed from average: {:.2f} standard deviations".format(
        (quote_upload[0] - acceptable_upload) / quote_upload[1]
    )
)

print(
    "{:.2f}% of measurements are lower than the acceptable download speed.".format(
        np.sum(df_compact["Download (Mbit/s)"] < acceptable_download)
        / float(len(df_compact))
        * 100
    )
)
...
print(
    "{:.2f}% of measurements are higher than the acceptable ping speed.".format(
        np.sum(df_compact["Ping (ms)"] > acceptable_ping) / float(len(df_compact)) * 100
    )
)

print(
    "{:.2f}% of measurements are lower than the acceptable upload speed.".format(
        np.sum(df_compact["Upload (Mbit/s)"] < acceptable_upload)
        / float(len(df_compact))
        * 100
    )
)
...
all_three = np.sum(
    (df_compact["Ping (ms)"] > acceptable_ping)
    & (df_compact["Download (Mbit/s)"] < acceptable_download)
    & (df_compact["Upload (Mbit/s)"] < acceptable_upload)
)
print(
    "{:.2f}% of measurements are not acceptable in three cases.".format(
        all_three / float(len(df_compact)) * 100
    )
)
...
print(
    "{:.2f}% of measurements are not acceptable for ping and upload.".format(
        ping_upload / float(len(df_compact)) * 100
    )
)

# ping_download = ...
ping_download = np.sum(
    (df_compact["Ping (ms)"] > acceptable_ping)
    & (df_compact["Download (Mbit/s)"] < acceptable_download)
)
...
# print(...)
print(
    "{:.2f}% of measurements are not acceptable for ping and download.".format(
        ping_download / float(len(df_compact)) * 100
    )
)

# upload_download = ...
upload_download = np.sum(
    (df_compact["Download (Mbit/s)"] < acceptable_download)
    & (df_compact["Upload (Mbit/s)"] < acceptable_upload)
)
...
# print(...)
print(
    "{:.2f}% of measurements are not acceptable for upload and download.".format(
        upload_download / float(len(df_compact)) * 100
    )
)
