...
for face in faces:
    ...
    ax.add_artist(
        Rectangle((x_smile, y_smile), w_smile, h_smile, fill=False, lw=3, color="green")
    )
...
# select only the face portion from the smile test image
cut_face_smile = gray_smile[
    y_smile + v_cut : y_smile + h_smile, x_smile + h_cut : x_smile - h_cut + w_smile
]


# select only the face portion from the no smile test image
face_no_smile = gray_no_smile[
    y_no_smile : y_no_smile + h_no_smile, x_no_smile : x_no_smile + w_no_smile
]
...
# select only the face portion from the smile test image
cut_face_no_smile = gray_no_smile[
    y_no_smile + v_cut : y_no_smile + h_no_smile,
...
standardized_face_smile = zoom(
    cut_face_smile, (64.0 / cut_face_smile.shape[0], 64.0 / cut_face_smile.shape[1])
).astype(np.float32)
...
standardized_face_no_smile = zoom(
    cut_face_no_smile,
    (64.0 / cut_face_no_smile.shape[0], 64.0 / cut_face_no_smile.shape[1]),
).astype(np.float32)
